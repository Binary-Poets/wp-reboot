...
jQuery(document).ready(function($) {
  ...
  $('#wp-reboot-tabs')
    .tabs({
      ...
      activate: function(event, ui) {
        localStorage.setItem('wp-reboot-tabs', $('#wp-reboot-tabs').tabs('option', 'active'));
      },
      active: localStorage.getItem('wp-reboot-tabs') || 0
    })
    .show();

  // helper for swithcing tabs & linking anchors in different tabs
  $('.tools_page_wp-reboot').on('click', '.change-tab', function(e) {
    e.preventDefault();

    $('#wp-reboot-tabs').tabs('option', 'active', $(this).data('tab'));
  ...
  $('.tools_page_wp-reboot').on('click', '.scrollto', function(e) {
    e.preventDefault();
  ...
  $('.tools_page_wp-reboot').on('click', '.button.dropdown-toggle', function(e) {
    e.preventDefault();
  ...
  // delete transients
  $('.tools_page_wp-reboot').on('click', '#delete-transients', 'click', function(e) {
    e.preventDefault();
  ...
  // delete uploads
  $('.tools_page_wp-reboot').on('click', '#delete-uploads', 'click', function(e) {
    e.preventDefault();
  ...
  // reboot theme options (mods)
  $('.tools_page_wp-reboot').on('click', '#reboot-theme-options', 'click', function(e) {
    e.preventDefault();

    run_tool(this, 'reboot_theme_options');

    return false;
  }); // reboot theme options

  // delete themes
  $('.tools_page_wp-reboot').on('click', '#delete-themes', 'click', function(e) {
    e.preventDefault();
  ...
  // delete plugins
  $('.tools_page_wp-reboot').on('click', '#delete-plugins', 'click', function(e) {
    e.preventDefault();
  ...
  // drop custom tables
  $('.tools_page_wp-reboot').on('click', '#drop-custom-tables', 'click', function(e) {
    e.preventDefault();
  ...
  $('.tools_page_wp-reboot').on('click', '#truncate-custom-tables', 'click', function(e) {
    e.preventDefault();
  ...
  $('.tools_page_wp-reboot').on('click', '#delete-htaccess', 'click', function(e) {
    e.preventDefault();
  ...
  $('#wpr-snapshots').on('click', '.compare-snapshot', 'click', function(e) {
    ...
    $.get({
      url: ajaxurl,
      data: {
        action: 'wp_reboot_run_tool',
        _ajax_nonce: wp_reboot.nonce_run_tool,
      ...
      .done(function(data) {
        if (data.success) {
          ...
          swal({
            type: 'error',
            title: wp_reboot.documented_error + ' ' + data.data
          });
        }
      })
      .fail(function(data) {
        swal({ type: 'error', title: wp_reboot.undocumented_error });
      });
  ...
  // create snapshot
  $('.tools_page_wp-reboot').on('click', '.create-new-snapshot', 'click', function(e) {
    e.preventDefault();
    ...
    swal({
      ...
      cancelButtonText: wp_reboot.cancel_button,
      width: 600
    }).then(result => {
  ...
  $('body.tools_page_wp-reboot').on('click', '.header-row', function(e) {
    e.preventDefault();
  ...
  function run_tool(button, tool_name, extra_data) {
    var confirm_title = $(button).data('confirm-title') || wp_reboot.confirm_title;
  ...
  $('#wp_reboot_form').on('submit', function(e, confirmed) {
    if (!confirmed) {
      $('#wp_reboot_submit').trigger('click');
      e.preventDefault();
      return false;
    }
  ...
  $('#wp_reboot_submit').click(function(e) {
    if ($('#wp_reboot_confirm').val() !== 'reboot') {
      swal({
        title: wp_reboot.invalid_confirmation_title,
        text: wp_reboot.invalid_confirmation,
        type: 'error',
        confirmButtonText: wp_reboot.ok_button
      });
    ...
    message = wp_reboot.confirm1 + '<br>' + wp_reboot.confirm2;
    swal({
      title: wp_reboot.confirm_title_reboot,
      type: 'question',
      html: message,
      showCancelButton: true,
      focusConfirm: false,
      focusCancel: true,
      confirmButtonText: wp_reboot.confirm_button,
      cancelButtonText: wp_reboot.cancel_button,
      confirmButtonColor: '#dd3036',
      width: 600
    }).then(result => {
      if (result.value === true) {
        block_ui(wp_reboot.doing_reboot);
        $('#wp_reboot_form').trigger('submit', true);
      }
    });
  ...
  $('.tools_page_wp-reboot').on('click', '.toggle-card', function(e, skip_anim) {
    e.preventDefault();
    ...
    cards = localStorage.getItem('wp-reboot-cards');
    if (cards == null) {
      cards = new Object();
    } else {
    ...
    localStorage.setItem('wp-reboot-cards', JSON.stringify(cards));
  ...
  $('.wpr-dismiss-notice').on('click', function(e) {
    ...
    $.get(ajaxurl, {
      ...
      _ajax_nonce: wp_reboot.nonce_dismiss_notice,
  ...
  $('.submit-survey').on('click', function(e) {
    ...
    $.post(ajaxurl, {
      ...
      _ajax_nonce: wp_reboot.nonce_submit_survey,
      action: 'wp_reboot_submit_survey'
    });
  ...
  $('.tools_page_wp-reboot').on('click', '.open-webhooks-dialog', function(e) {
    $(this).blur();
  ...
  $('#webhooks-dialog').dialog({
    ...
    open: function(event, ui) {
      ...
      $(this)
        ...
        .html(wp_reboot.webhooks_dialog_title);
    },
    close: function(event, ui) {},
    autoOpen: false,
    closeOnEscape: true
  ...
  jQuery('#install-webhooks').on('click', function(e) {
    ...
    jQuery('body').append(
      '<div style="width:550px;height:450px; position:fixed;top:10%;left:50%;margin-left:-275px; color:#444; background-color: #fbfbfb;border:1px solid #DDD; border-radius:4px;box-shadow: 0px 0px 0px 4000px rgba(0, 0, 0, 0.85);z-index: 9999999;"><iframe src="' +
        wp_reboot.webhooks_install_url +
