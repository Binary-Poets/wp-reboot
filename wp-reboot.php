<?php
/*
  Plugin Name: WP Reboot
  Plugin URI: https://www.binarypoets.net/wp-reboot
  Description: Advanced WordPress Reboot Tool.
  Version: 1.0.1
  Author: BinaryPoets
  Author URI: https://www.binarypoets.net
  Text Domain: wp-reboot

  Copyright 2015 - 2019  Web factory Ltd  (email: hello@binarypoets.net)
...
if (defined('WP_CLI') && WP_CLI) {
  require_once dirname(__FILE__) . '/wp-reboot-cli.php';
}
...
class WP_Reboot
{
  ...
  public $plugin_url = '';
  public $plugin_dir = '';
  public $snapshots_folder = 'wp-reboot-snapshots-export';
  protected $options = array();
  private $delete_count = 0;
  private $licensing_servers = array('https://www.binarypoets.net/legal1/', 'https://www.binarypoets.net/legal2/');
  ...
  /**
   * Creates a new WP_Reboot object and implements singleton
   *
   * @return WP_Reboot
   */
  static function getInstance()
  {
    if (!is_a(self::$instance, 'WP_Reboot')) {
      self::$instance = new WP_Reboot();
    }
  ...
  private function __construct()
  {
    ...
    add_action('wp_ajax_wp_reboot_dismiss_notice', array($this, 'ajax_dismiss_notice'));
    add_action('wp_ajax_wp_reboot_run_tool', array($this, 'ajax_run_tool'));
    add_action('wp_ajax_wp_reboot_submit_survey', array($this, 'ajax_submit_survey'));
  ...
  {
    $options = get_option('wp-reboot', array());
    $change = false;

    if (!isset($options['meta'])) {
      $options['meta'] = array('first_version' => $this->version, 'first_install' => current_time('timestamp', true), 'reboot_count' => 0);
      $change = true;
    }
    ...
    if ($change) {
      update_option('wp-reboot', $options, true);
    }
  ...
  function update_options($key, $data)
  {
    ...
    $tmp = update_option('wp-reboot', $this->options);
  ...
  function admin_menu()
  {
    add_management_page(__('WP Reboot', 'wp-reboot'), __('WP Reboot', 'wp-reboot'), 'administrator', 'wp-reboot', array($this, 'plugin_page'));
  } // admin_menu
  ...
  {
    check_ajax_referer('wp-reboot_dismiss_notice');

    if (!current_user_can('administrator')) {
      wp_send_json_error(__('You are not allowed to run this action.', 'wp-reboot'));
    }

    $notice_name = trim(@$_GET['notice_name']);
    if (!$this->dismiss_notice($notice_name)) {
      wp_send_json_error(__('Notice is already dismissed.', 'wp-reboot'));
    } else {
  ...
  function get_pointers()
  {
    ...
    $pointers['welcome'] = array('target' => '#menu-tools', 'edge' => 'left', 'align' => 'right', 'content' => 'Thank you for installing the <b style="font-weight: 800;">WP Reboot</b> plugin!<br>Open <a href="' . admin_url('tools.php?page=wp-reboot') . '">Tools - WP Reboot</a> to access rebootting tools and start developing &amp; debugging faster.');
  ...
  function admin_enqueue_scripts($hook)
  {
    ...
    if (!empty($pointers) && !$this->is_plugin_page() && current_user_can('administrator')) {
      $pointers['_nonce_dismiss_pointer'] = wp_create_nonce('wp-reboot_dismiss_notice');

      wp_enqueue_style('wp-pointer');

      wp_enqueue_script('wp-reboot-pointers', $this->plugin_url . 'js/wp-reboot-pointers.js', array('jquery'), $this->version, true);
      wp_enqueue_script('wp-pointer');
      wp_localize_script('wp-pointer', 'wp_reboot_pointers', $pointers);
    }

    // exit early if not on WP Reboot page
    if (!$this->is_plugin_page()) {
      return;
    }

    // features survey is shown 5min after install or after first reboot
    $survey = false;
    ...
    $js_localize = array(
      'undocumented_error' => __('An undocumented error has occurred. Please refresh the page and try again.', 'wp-reboot'),
      'documented_error' => __('An error has occurred.', 'wp-reboot'),
      'plugin_name' => __('WP Reboot', 'wp-reboot'),
      'settings_url' => admin_url('tools.php?page=wp-reboot'),
      'icon_url' => $this->plugin_url . 'img/wp-reboot-icon.png',
      'invalid_confirmation' => __('Please type "reboot" in the confirmation field.', 'wp-reboot'),
      'invalid_confirmation_title' => __('Invalid confirmation', 'wp-reboot'),
      'cancel_button' => __('Cancel', 'wp-reboot'),
      'open_survey' => $survey,
      'ok_button' => __('OK', 'wp-reboot'),
      'confirm_button' => __('Reboot WordPress', 'wp-reboot'),
      'confirm_title' => __('Are you sure you want to proceed?', 'wp-reboot'),
      'confirm_title_reboot' => __('Are you sure you want to reboot the site?', 'wp-reboot'),
      'confirm1' => __('Clicking "Reboot WordPress" will reboot your site to default values. All content will be lost. Always <a href="#" class="create-new-snapshot" data-description="Before rebootting the site">create a snapshot</a> if you want to be able to undo.</b>', 'wp-reboot'),
      'confirm2' => __('Click "Cancel" to abort.', 'wp-reboot'),
      'doing_reboot' => __('Rebootting in progress. Please wait.', 'wp-reboot'),
      'nonce_dismiss_notice' => wp_create_nonce('wp-reboot_dismiss_notice'),
      'nonce_run_tool' => wp_create_nonce('wp-reboot_run_tool'),
      'nonce_do_reboot' => wp_create_nonce('wp-reboot_do_reboot'),
    );

    if ($survey) {
      $js_localize['nonce_submit_survey'] = wp_create_nonce('wp-reboot_submit_survey');
    }
    ...
    wp_enqueue_style('wp-reboot', $this->plugin_url . 'css/wp-reboot.css', array(), $this->version);
    wp_enqueue_style('wp-reboot-sweetalert2', $this->plugin_url . 'css/sweetalert2.min.css', array(), $this->version);

    wp_enqueue_script('plugin-install');
    wp_enqueue_script('jquery-ui-dialog');
    wp_enqueue_script('jquery-ui-tabs');
    wp_enqueue_script('wp-reboot-sweetalert2', $this->plugin_url . 'js/wp-reboot-libs.min.js', array('jquery'), $this->version, true);
    wp_enqueue_script('wp-reboot', $this->plugin_url . 'js/wp-reboot.js', array('jquery'), $this->version, true);
    wp_localize_script('wp-reboot', 'wp_reboot', $js_localize);
  ...
  {
    check_ajax_referer('wp-reboot_submit_survey');
  ...
  function is_survey_active($survey_name)
  {
    ...
    $meta = $this->get_meta();
    if (current_time('timestamp', true) - $meta['first_install'] > 300 || $meta['reboot_count'] > 0) {
      return true;
    }
  ...
  {
    if (!is_null($value = apply_filters('wp-reboot-override-is-cli-running', null))) {
      return (bool) $value;
    }
  ...
  function is_webhooks_active()
  {
    ...
    if (false == is_plugin_active('wpwh-wp-reboot-webhook-integration/wpwhpro-wp-reboot-webhook-integration.php')) {
      return false;
    }
  ...
  static function install_webhooks()
  {
    ...
    $plugin_slug = 'wpwh-wp-reboot-webhook-integration/wpwhpro-wp-reboot-webhook-integration.php';
    $plugin_zip = 'https://downloads.wordpress.org/plugin/wpwh-wp-reboot-webhook-integration.latest-stable.zip';
    ...
    if (!is_wp_error($installed) && $installed) {
      ...
      if (is_null($activate)) {
        ...
        echo '<script>setTimeout(function() { top.location = "tools.php?page=wp-reboot"; }, 1000);</script>';
        echo '<br>If you are not redirected in a few seconds - <a href="tools.php?page=wp-reboot" target="_parent">click here</a>.';
      }
    } else {
  ...
  function do_delete_transients()
  {
    ...
    do_action('wp_reboot_delete_transients', $count);
  ...
  function do_delete_uploads()
  {
    ...
    do_action('wp_reboot_delete_uploads', $this->delete_count);
   ...
   *
   * @param bool  $keep_wp_reboot  Keep WP Reboot active and installed
   ...
   */
  function do_delete_plugins($keep_wp_reboot = true, $silent_deactivate = false)
  {
    if (!function_exists('get_plugins')) {
      require_once ABSPATH . 'wp-admin/includes/plugin.php';
    }
    ...
    $wp_reboot_basename = plugin_basename(__FILE__);

    $all_plugins = get_plugins();
    $active_plugins = (array) get_option('active_plugins', array());
    if (true == $keep_wp_reboot) {
      if (($key = array_search($wp_reboot_basename, $active_plugins)) !== false) {
        unset($active_plugins[$key]);
      }
      unset($all_plugins[$wp_reboot_basename]);
    }
    ...
    do_action('wp_reboot_delete_plugins', $all_plugins, $all_plugins);
  ...
  function do_delete_themes($keep_default_theme = true)
  {
    ...
    do_action('wp_reboot_delete_themes', $all_themes);
  ...
  function do_truncate_custom_tables()
  {
    ...
    do_action('wp_reboot_truncate_custom_tables', $custom_tables);
  ...
  function do_drop_custom_tables()
  {
    ...
    do_action('wp_reboot_drop_custom_tables', $custom_tables);
  ...
  function do_delete_htaccess()
  {
    ...
    do_action('wp_reboot_delete_htaccess', $htaccess_path);
  ...
  {
    check_ajax_referer('wp-reboot_run_tool');
    ...
    if ($tool == 'delete_transients') {
    ...
    } elseif ($tool == 'reboot_theme_options') {
      $cnt = $this->do_reboot_theme_options(true);
      wp_send_json_success($cnt);
      ...
      wp_send_json_error(__('Unknown tool.', 'wp-reboot'));
    }
  } // ajax_run_tool


  /**
   * Reinstall / reboot the WP site
  ...
  function do_reinstall($params = array())
  {
    ...
    // only admins can reboot; double-check
    ...
    // save values that need to be restored after reboot
    // todo: use params to determine what gets restored after reboot
    ...
    if (!$current_user->ID) {
      ...
      if (empty($tmp[0]->user_login)) {
        return new WP_Error(1, 'Reboot failed. Unable to find any admin users in database.');
      }
      $current_user = $tmp[0];
    }
    ...
    // todo: find a better way to supress errors and send/not send email on reboot
    ...
    // restore rest of the settings including WP Reboot's
    update_option('siteurl', $siteurl);
    update_option('home', $home);
    update_option('wp-reboot', $this->options);
    update_option('wp-reboot-snapshots', $snapshots);
    ...
    // reactivate WP Reboot
    if (!empty($params['reactivate_wpreboot'])) {
    ...
    if (!empty($params['reactivate_webhooks'])) {
      ...
      activate_plugin('wpwh-wp-reboot-webhook-integration/wpwhpro-wp-reboot-webhook-integration.php');
    ...
    if (!$this->is_cli_running()) {
      ...
      wp_redirect(admin_url() . '?wp-reboot=success');
      exit;
    }
  } // do_reinstall
  ...
  /**
   * Checks wp_reboot post value and performs all actions
  ...
  function do_all_actions()
  {
    ...
    if (!empty($_GET['wp-reboot']) && $_GET['wp-reboot'] == 'success') {
      add_action('admin_notices', array($this, 'notice_successful_reboot'));
    }

    // check nonce
    if (true === isset($_POST['wp_reboot_confirm']) && false === wp_verify_nonce(@$_POST['_wpnonce'], 'wp-reboot')) {
      add_settings_error('wp-reboot', 'bad-nonce', __('Something went wrong. Please refresh the page and try again.', 'wp-reboot'), 'error');
      return false;
    }

    // check confirmation code
    if (true === isset($_POST['wp_reboot_confirm']) && 'reboot' !== $_POST['wp_reboot_confirm']) {
      add_settings_error('wp-reboot', 'bad-confirm', __('<b>Invalid confirmation code.</b> Please type "reboot" in the confirmation field.', 'wp-reboot'), 'error');
      return false;
    }
    ...
    if (true === isset($_POST['wp_reboot_confirm']) && 'reboot' === $_POST['wp_reboot_confirm']) {
      $defaults = array(
        ...
        'reactivate_wpreboot' => '0',
        'reactivate_webhooks' => '0'
      );
      $params = shortcode_atts($defaults, (array) @$_POST['wpr-post-reboot']);
  ...
  /**
   * Add "Open WP Reboot Tools" action link to plugins table, left part
   *
  ...
  {
    $settings_link = '<a href="' . admin_url('tools.php?page=wp-reboot') . '" title="' . __('Open WP Reboot Tools', 'wp-reboot') . '">' . __('Open WP Reboot Tools', 'wp-reboot') . '</a>';
  ...
  function plugin_meta_links($links, $file)
  {
    ...
    $support_link = '<a target="_blank" href="https://wordpress.org/support/plugin/wp-reboot" title="' . __('Get help', 'wp-reboot') . '">' . __('Support', 'wp-reboot') . '</a>';
    $home_link = '<a target="_blank" href="' . $this->generate_web_link('plugins-table-right') . '" title="' . __('Plugin Homepage', 'wp-reboot') . '">' . __('Plugin Homepage', 'wp-reboot') . '</a>';
    $rate_link = '<a target="_blank" href="https://wordpress.org/support/plugin/wp-reboot/reviews/#new-post" title="' . __('Rate the plugin', 'wp-reboot') . '">' . __('Rate the plugin ★★★★★', 'wp-reboot') . '</a>';
  ...
  function is_plugin_page()
  {
    ...
    if ($current_screen->id == 'tools_page_wp-reboot') {
      return true;
    } else {
      return false;
    }
  ...
  function admin_footer_text($text)
  {
    ...
    $text = '<i><a href="' . $this->generate_web_link('admin_footer') . '" title="' . __('Visit WP Reboot page for more info', 'wp-reboot') . '" target="_blank">WP Reboot</a> v' . $this->version . '. Please <a target="_blank" href="https://wordpress.org/support/plugin/wp-reboot/reviews/#new-post" title="Rate the plugin">rate the plugin <span>★★★★★</span></a> to help us spread the word. Thank you from the WP Reboot team!</i>';
  ...
  {
    load_plugin_textdomain('wp-reboot');
  } // load_textdomain


  /**
   * Inform the user that WordPress has been successfully reboot
   *
   * @return null
   */
  function notice_successful_reboot()
  {
    global $current_user;

    echo '<div style="padding: 15px; display: inline-block; font-size: 14px;" id="message" class="updated"><p style="font-size: 14px;">' . sprintf(__('<b>Site has been successfully reboot to default settings.</b><br>User "%s" was restored with the password unchanged. Open <a href="%s">WP Reboot</a> to do another reboot.', 'wp-reboot'), $current_user->user_login, admin_url('tools.php?page=wp-reboot')) . '</p>';
    ...
    if (false == $this->get_dismissed_notices('rate')) {
      ...
      echo 'If WP Reboot helped you please rate it so we can continue supporting it and helping others. Thank you!<br>';
      echo '<a style="margin-top: 5px;" class="button button-secondary" href="https://wordpress.org/support/plugin/wp-reboot/reviews/?filter=5#new-post" target="_blank">You deserve it, I\'ll rate it!</a> &nbsp; &nbsp; <a href="' . $dismiss_url . '">I already rated it</a>';
      echo '</p>';
    }

    echo '</div>';
  } // notice_successful_reboot
  ...
  function get_card_header($title, $card_id, $collapse_button = false, $iot_button = false)
  {
    ...
    if ($collapse_button) {
      $out .= '<a class="toggle-card" href="#" title="' . __('Collapse / expand box', 'wp-reboot') . '"><span class="dashicons dashicons-arrow-up-alt2"></span></a>';
    }
    $out .= '</div></h4>';
  ...
  function plugin_page()
  {
    ...
    if (!current_user_can('administrator')) {
      wp_die(__('Sorry, you are not allowed to access this page.', 'wp-reboot'));
    }

    echo '<div class="wrap">';
    echo '<form id="wp_reboot_form" action="' . admin_url('tools.php?page=wp-reboot') . '" method="post" autocomplete="off">';

    echo '<header>';
    echo '<div class="wpr-container">';
    echo '<img id="logo-icon" src="' . $this->plugin_url . 'img/wp-reboot-logo.png" title="' . __('WP Reboot', 'wp-reboot') . '" alt="' . __('WP Reboot', 'wp-reboot') . '">';
    echo '</div>';
    echo '</header>';

    echo '<div id="loading-tabs"><img class="rotating" src="' . $this->plugin_url . 'img/wp-reboot-icon.png' . '" alt="Loading. Please wait." title="Loading. Please wait."></div>';

    echo '<div id="wp-reboot-tabs" class="ui-tabs" style="display: none;">';

    echo '<nav>';
    echo '<div class="wpr-container">';
    echo '<ul class="wpr-main-tab">';
    echo '<li><a href="#tab-reboot">' . __('Reboot', 'wp-reboot') . '</a></li>';
    echo '<li><a href="#tab-tools">' . __('Tools', 'wp-reboot') . '</a></li>';
    echo '<li><a href="#tab-snapshots">' . __('Snapshots', 'wp-reboot') . '</a></li>';
    echo '<li><a href="#tab-collections">' . __('Collections', 'wp-reboot') . '</a></li>';
    echo '<li><a href="#tab-support">' . __('Support', 'wp-reboot') . '</a></li>';
    echo '</ul>';
    echo '</div>'; // container
    echo '</nav>';
    ...
    echo '<div style="display: none;" id="tab-reboot">';
    $this->tab_reboot();
    echo '</div>';
    ...
    // survey
    if ($this->is_survey_active('features')) {
      echo '<div id="survey-dialog" style="display: none;" title="Help us make WP Reboot better for you"><span class="ui-helper-hidden-accessible"><input type="text"/></span>';
      ...
      $questions[] = '<div class="question-wrapper" data-value="wpmu" title="Click to select/unselect answer">' .
        ...
        '<i>Full support &amp; compatibility for all WP Reboot tools for all sites in network</i></div>' .
        '</div>';
      ...
      $questions[] = '<div class="question-wrapper" data-value="nothing" title="Click to select/unselect answer">' .
        ...
        '<i>WP Reboot is perfect as is - I don\'t need any new features</i></div>' .
        '</div>';

      $questions[] = '<div class="question-wrapper" data-value="nuclear" title="Click to select/unselect answer">' .
        '<span class="dashicons dashicons-yes"></span>' .
        '<div class="question"><b>Nuclear reboot - run all tools at once</b><br>' .
        '<i>Besides rebootting, delete all files and all other customizations with one click</i></div>' .
        '</div>';

      $questions[] = '<div class="question-wrapper" data-value="plugin-collections" title="Click to select/unselect answer">' .
        '<span class="dashicons dashicons-yes"></span>' .
        '<div class="question"><b>Install a set of plugins/themes after reboot</b><br>' .
        '<i>Save lists of plugins/themes and automatically install them after rebootting</i></div>' .
        '</div>';
  ...
  private function custom_notifications()
  {
    ...
    if (false === $notice_shown && is_multisite()) {
      ...
      echo '<h2>' . __('WP Reboot is not compatible with multisite!', 'wp-reboot') . '</h2>';
      echo '<p>' . __('Please be careful when using WP Reboot with multisite enabled. It\'s not recommended to reboot the main site. Sub-sites should be OK. We\'re working on making it fully compatible with WP-MU. <b>Till then please be careful.</b> Thank you for understanding.', 'wp-reboot') . '</p>';
      echo '</div>';
      $notice_shown = true;
    }

    // ask for review
    if ((!empty($meta['reboot_count']) || !empty($snapshots) || current_time('timestamp', true) - $meta['first_install'] > DAY_IN_SECONDS)
      && false === $notice_shown
      && false == $this->get_dismissed_notices('rate')
    ) {
      echo '<div class="card notice-wrapper notice-info">';
      echo '<h2>' . __('Please help us spread the word &amp; keep the plugin up-to-date', 'wp-reboot') . '</h2>';
      echo '<p>' . __('If you use &amp; enjoy WP Reboot, <b>please rate it on WordPress.org</b>. It only takes a second and helps us keep the plugin maintained. Thank you!', 'wp-reboot') . '</p>';
      echo '<p><a class="button-primary button" title="' . __('Rate WP Reboot', 'wp-reboot') . '" target="_blank" href="https://wordpress.org/support/plugin/wp-reboot/reviews/?filter=5#new-post">' . __('Rate the plugin ★★★★★', 'wp-reboot') . '</a>  <a href="#" class="wpr-dismiss-notice dismiss-notice-rate" data-notice="rate">' . __('I\'ve already rated it', 'wp-reboot') . '</a></p>';
      echo '</div>';
      $notice_shown = true;
    }
  ...
  /**
   * Echoes content for reboot tab
   *
   * @return null
   */
  private function tab_reboot()
  {
    global $current_user, $wpdb;

    echo '<div class="card" id="card-description">';
    echo '<h4>';
    echo __('Please read carefully before proceeding', 'wp-reboot');
    echo '<div class="card-header-right"><a class="toggle-card" href="#" title="' . __('Collapse / expand box', 'wp-reboot') . '"><span class="dashicons dashicons-arrow-up-alt2"></span></a></div>';
    echo '</h4>';
    echo '<div class="card-body">';
    echo '<p><b class="red">' . __('Rebootting will delete:', 'wp-reboot') . '</b></p>';
    echo '<ul class="plain-list">';
    echo '<li>' . __('all posts, pages, custom post types, comments, media entries, users', 'wp-reboot') . '</li>';
    echo '<li>' . __('all default WP database tables', 'wp-reboot') . '</li>';
    echo '<li>' . sprintf(__('all custom database tables that have the same prefix "%s" as default tables in this installation', 'wp-reboot'), $wpdb->prefix) . '</li>';
    echo '<li>' . __('always <a href="#" class="create-new-snapshot">create a snapshot</a> or a full backup, so you can restore it later', 'wp-reboot') . '</li>';
    echo '</ul>';

    echo '<p><b class="green">' . __('Rebootting will not delete:', 'wp-reboot') . '</b></p>';
    echo '<ul class="plain-list">';
    echo '<li>' . __('media files - they\'ll remain in the <i>wp-uploads</i> folder but will no longer be listed under Media', 'wp-reboot');
    echo __('; use the <a href="#tool-delete-uploads" data-tab="1" class="change-tab">Clean Uploads Folder</a> tool to remove media files', 'wp-reboot') . '</li>';
    echo '<li>' . __('no files are touched; plugins, themes, uploads - everything stays', 'wp-reboot');
    echo __('; if needed use the <a href="#tool-delete-themes" class="change-tab" data-tab="1">Delete Themes</a> &amp; <a href="#tool-delete-plugins" class="change-tab" data-tab="1">Delete Plugins</a> tools', 'wp-reboot') . '</li>';
    echo '<li>' . __('site title, WordPress address, site address, site language and search engine visibility settings', 'wp-reboot') . '</li>';
    echo '<li>' . sprintf(__('logged in user "%s" will be restored with the current password', 'wp-reboot'), $current_user->user_login) . '</li>';
    echo '</ul>';

    echo '<p><b>' . __('What happens when I click the Reboot button?', 'wp-reboot') . '</b></p>';
    echo '<ul class="plain-list">';
    echo '<li>' . __('remember, always <a href="#" class="create-new-snapshot">make a snapshot</a> or a full backup first', 'wp-reboot') . '</li>';
    echo '<li>' . __('you will have to confirm the action one more time', 'wp-reboot') . '</li>';
    echo '<li>' . __('everything will be reboot; see bullets above for details', 'wp-reboot') . '</li>';
    echo '<li>' . __('site title, WordPress address, site address, site language, search engine visibility and current user will be restored', 'wp-reboot') . '</li>';
    echo '<li>' . __('you will be logged out, automatically logged in and taken to the admin dashboard', 'wp-reboot') . '</li>';
    echo '<li>' . __('WP Reboot plugin will be reactivated if that option is chosen in the <a href="#card-reboot">post-reboot options</a>', 'wp-reboot') . '</li>';
    echo '</ul>';

    echo '<p><b>' . __('WP-CLI Support', 'wp-reboot') . '</b><br>';
    echo '' . sprintf(__('All tools available via GUI are available in WP-CLI as well. To get the list of commands run %s. Instead of the active user, the first user with admin privileges found in the database will be restored. ', 'wp-reboot'), '<code>wp help reboot</code>');
    echo sprintf(__('All actions have to be confirmed. If you want to skip confirmation use the standard %s option. Please be careful - there is NO UNDO.', 'wp-reboot'), '<code>--yes</code>') . '</p>';

    echo '<p><b>' . __('WP Webhooks Support', 'wp-reboot') . '</b><br>';
    echo 'All WP Reboot tools are integrated with <a href="https://wordpress.org/plugins/wp-webhooks/" target="_blank">WP Webhooks</a> and available as (receive data) actions. Webhooks are a standard, platform-independent way of connecting WordPress to any 3rd party system. This <a href="https://underconstructionpage.com/wp-webhooks-connect-integrate-wordpress/" target="_blank">article</a> has more info, videos and use-cases so you can see just how powerful and easy to use webhooks are.<br>';
    if ($this->is_webhooks_active()) {
      echo 'WP Webhooks are active. Make sure you enable WP Reboot actions in <a href="' . admin_url('options-general.php?page=wp-webhooks-pro&wpwhvrs=settings') . '">settings</a>.';
    } else {
    ...
    echo '<div class="card" id="card-reboot">';
    echo '<h4>' . __('Site Reboot', 'wp-reboot');
    echo '<div class="card-header-right"><a class="toggle-card" href="#" title="' . __('Collapse / expand box', 'wp-reboot') . '"><span class="dashicons dashicons-arrow-up-alt2"></span></a></div>';
    echo '</h4>';
    echo '<div class="card-body">';
    echo '<p><label for="reactivate-theme"><input name="wpr-post-reboot[reactivate_theme]" type="checkbox" id="reactivate-theme" value="1"> ' . __('Reactivate current theme', 'wp-reboot') . ' - ' . $theme->get('Name') . '</label></p>';
    echo '<p><label for="reactivate-wpreboot"><input name="wpr-post-reboot[reactivate_wpreboot]" type="checkbox" id="reactivate-wpreboot" value="1" checked> ' . __('Reactivate WP Reboot plugin', 'wp-reboot') . '</label></p>';
    if ($this->is_webhooks_active()) {
      echo '<p><label for="reactivate-webhooks"><input name="wpr-post-reboot[reactivate_webhooks]" type="checkbox" id="reactivate-webhooks" value="1" checked> ' . __('Reactivate WP Webhooks plugin', 'wp-reboot') . '</label></p>';
    }
    echo '<p><label for="reactivate-plugins"><input name="wpr-post-reboot[reactivate_plugins]" type="checkbox" id="reactivate-plugins" value="1"> ' . __('Reactivate all currently active plugins', 'wp-reboot') . '</label></p>';
    if ($this->is_webhooks_active()) {
      echo '<p><a href="' . admin_url('options-general.php?page=wp-webhooks-pro&wpwhvrs=settings') . '">Configure WP Webhooks</a> to run additional actions after reboot, or connect to any 3rd party system.</p>';
    } else {
      echo '<p>To run additional actions after reboot or automate a complex workflow <a href="#" class="open-webhooks-dialog">install WP Webhooks &amp; WPR addon</a>. It\'s a standard, platform-independent way of connecting WordPress to any web app. This <a href="https://www.youtube.com/watch?v=m8XDFXCNP9g" target="_blank">short video</a> explains it well.</p>';
    }
    echo '<p><br>' . __('Type <b>reboot</b> in the confirmation field to confirm the reboot and then click the "Reboot WordPress" button.<br>Always <a href="#" class="create-new-snapshot" data-description="Before rebootting the site">create a snapshot</a> before rebootting if you want to be able to undo.', 'wp-reboot') . '</p>';

    wp_nonce_field('wp-reboot');
    echo '<p><input id="wp_reboot_confirm" type="text" name="wp_reboot_confirm" placeholder="' . esc_attr__('Type in "reboot"', 'wp-reboot') . '" value="" autocomplete="off"> &nbsp;';
    echo '<a id="wp_reboot_submit" class="button button-delete">' . __('Reboot Site', 'wp-reboot') . '</a>' . $this->get_snapshot_button('reboot-wordpress', 'Before rebootting the site') . '</p>';
    echo '</div>';
    echo '</div>';
  } // tab_reboot
  ...
  private function tab_tools()
  {
    ...
    $tools = array(
      ...
      'tool-reboot-theme-options' => 'Reboot Theme Options',
    ...
    echo '<div class="card">';
    echo $this->get_card_header(__('Index of Tools', 'wp-reboot'), 'iot', true, false);
    echo '<div class="card-body">';
    $i = 0;
    ...
    echo '<div class="card">';
    echo $this->get_card_header(__('Delete Transients', 'wp-reboot'), 'tool-delete-transients', true, true);
    echo '<div class="card-body">';
    ...
    echo '<div class="card">';
    echo $this->get_card_header(__('Clean Uploads Folder', 'wp-reboot'), 'tool-delete-uploads', true, true);
    echo '<div class="card-body">';
    echo '<p>' . __('All files in <code>' . $upload_dir['basedir'] . '</code> folder will be deleted. Including folders and subfolders, and files in subfolders. Files associated with <a href="' . admin_url('upload.php') . '">media</a> entries will be deleted too.<br><b>There is NO UNDO. WP Reboot does not make any file backups.</b>', 'wp-reboot') . '</p>';
    ...
    if (false != $upload_dir['error']) {
      ...
      echo '<p class="mb0"><a data-confirm-title="Are you sure you want to delete all files &amp; folders in uploads folder?" data-btn-confirm="Delete everything in uploads folder" data-text-wait="Deleting uploads. Please wait." data-text-confirm="All files and folders in uploads will be deleted. There is NO UNDO. WP Reboot does not make any file backups." data-text-done="%n files &amp; folders have been deleted." data-text-done-singular="One file or folder has been deleted." class="button button-delete" href="#" id="delete-uploads">Delete all files &amp; folders in uploads folder</a></p>';
    }
    echo '</div>';
    echo '</div>';

    echo '<div class="card">';
    echo $this->get_card_header(__('Reboot Theme Options', 'wp-reboot'), 'tool-reboot-theme-options', true, true);
    echo '<div class="card-body">';
    echo '<p>' . __('All options (mods) for all themes will be reboot; not just for the active theme. The tool works only for themes that use the <a href="https://codex.wordpress.org/Theme_Modification_API" target="_blank">WordPress theme modification API</a>. If options are saved in some other, custom way they won\'t be reboot.<br> Always <a href="#" class="create-new-snapshot" data-description="Before rebootting theme options">create a snapshot</a> before using this tool if you want to be able to undo its actions.', 'wp-reboot') . '</p>';
    echo $this->get_tool_icons(false, true);
    echo '<p class="mb0"><a data-confirm-title="Are you sure you want to reboot all theme options?" data-btn-confirm="Reboot theme options" data-text-wait="Rebootting theme options. Please wait." data-text-confirm="All options (mods) for all themes will be reboot. Always ' . esc_attr('<a data-description="Before rebootting theme options" href="#" class="create-new-snapshot">create a snapshot</a> if you want to be able to undo') . '." data-text-done="Options for %n themes have been reboot." data-text-done-singular="Options for one theme have been reboot." class="button button-delete" href="#" id="reboot-theme-options">Reboot theme options</a>' . $this->get_snapshot_button('reboot-theme-options', 'Before rebootting theme options') . '</p>';
    echo '</div>';
    echo '</div>';
    ...
    echo '<div class="card">';
    echo $this->get_card_header(__('Delete Themes', 'wp-reboot'), 'tool-delete-themes', true, true);
    echo '<div class="card-body">';
    echo '<p>' . __('All themes will be deleted. Including the currently active theme - ' . $theme->get('Name') . '.<br><b>There is NO UNDO. WP Reboot does not make any file backups.</b>', 'wp-reboot') . '</p>';

    echo $this->get_tool_icons(true, true);

    echo '<p class="mb0"><a data-confirm-title="Are you sure you want to delete all themes?" data-btn-confirm="Delete all themes" data-text-wait="Deleting all themes. Please wait." data-text-confirm="All themes will be deleted. There is NO UNDO. WP Reboot does not make any file backups." data-text-done="%n themes have been deleted." data-text-done-singular="One theme has been deleted." class="button button-delete" href="#" id="delete-themes">Delete all themes</a></p>';
    echo '</div>';
    echo '</div>';

    echo '<div class="card">';
    echo $this->get_card_header(__('Delete Plugins', 'wp-reboot'), 'tool-delete-plugins', true, true);
    echo '<div class="card-body">';
    echo '<p>' . __('All plugins will be deleted except for WP Reboot which will remain active.<br><b>There is NO UNDO. WP Reboot does not make any file backups.</b>', 'wp-reboot') . '</p>';

    echo $this->get_tool_icons(true, true);

    echo '<p class="mb0"><a data-confirm-title="Are you sure you want to delete all plugins?" data-btn-confirm="Delete plugins" data-text-wait="Deleting plugins. Please wait." data-text-confirm="All plugins except WP Reboot will be deleted. There is NO UNDO. WP Reboot does not make any file backups." data-text-done="%n plugins have been deleted." data-text-done-singular="One plugin has been deleted." class="button button-delete" href="#" id="delete-plugins">Delete plugins</a></p>';
    echo '</div>';
    echo '</div>';
    ...
    if ($custom_tables) {
      ...
      echo '<p>' . __('There are no custom tables. There\'s nothing for this tool to empty or delete.', 'wp-reboot') . '</p>';
    ...
    echo '<div class="card">';
    echo $this->get_card_header(__('Delete .htaccess File', 'wp-reboot'), 'tool-delete-htaccess', true, true);
    echo '<div class="card-body">';
    echo '<p>' . __('This action deletes the .htaccess file located in <code>' . $this->get_htaccess_path() . '</code><br><b>There is NO UNDO. WP Reboot does not make any file backups.</b></p>', 'wp-reboot');
  ...
  {
    echo '<div class="card">';
    echo '<h4>' . __('What are Plugin &amp; Theme Collections?', 'wp-reboot') . '</h4>';
    echo '<p>' . __('A tool that\'s coming with WP Reboot PRO that will <b>save hours &amp; hours of your precious time</b>! Have a set of plugins (and themes) that you install and activate after every reboot? Or on every fresh WP installation? Well, no more clicking install &amp; active for ten minutes! Build the collection once and install it with one click as many times as needed.<br>WP Reboot stores collections in the cloud so they\'re accessible on every site you build. You can use free plugins and themes from the official repo, and PRO ones by uploading a ZIP file. We\'ll safely store your license keys too, so you have everything in one place.', 'wp-reboot') . '</p>';
    echo '<p class="textcenter"><a class="button button-primary" href="https://www.binarypoets.net/wp-rebootltd-sale-martech-wise/" target="_blank"><b>Reserve your copy of WP Reboot PRO! It launches on January 13th!</b></a></p>';
    echo '</div>';
  } // tab_collections
  ...
  {
    echo '<div class="card">';
    echo '<h4>' . __('Documentation', 'wp-reboot') . '</h4>';
    echo '<p>' . __('All tools and functions are explained in detail in <a href="' . $this->generate_web_link('support-tab', '/documentation/') . '" target="_blank">the documentation</a>. We did our best to describe how things work on both the code level and an "average user" level.', 'wp-reboot') . '</p>';
    echo '</div>';

    echo '<div class="card">';
    echo '<h4>' . __('Public support forum', 'wp-reboot') . '</h4>';
    echo '<p>' . __('We are very active on the <a href="https://wordpress.org/support/plugin/wp-reboot" target="_blank">official WP Reboot support forum</a>. If you found a bug, have a feature idea or just want to say hi - please drop by. We love to hear back from our users.', 'wp-reboot') . '</p>';
    echo '</div>';

    echo '<div class="card">';
    echo '<h4>' . __('Care to help out?', 'wp-reboot') . '</h4>';
    echo '<p>' . __('No need for donations or anything like that :) If you can give us a <a href="https://wordpress.org/support/plugin/wp-reboot/reviews/#new-post" target="_blank">five star rating</a> you\'ll help out more than you can imagine. A public mention <a href="https://twitter.com/binarypoets" target="_blank">@binarypoets</a> also does wonders. Thank you!', 'wp-reboot') . '</p>';
    echo '</div>';
  } // tab_support
  ...
  private function tab_snapshots()
  {
    ...
    echo '<p>Snapshots are primarily a development tool. When using various reboot tools that alter the database be sure to create a snapshot. Shortcuts are available in the confirmation dialog. If you need a full backup that includes files, use a dedicated <a target="_blank" href="' . admin_url('plugin-install.php?s=backup&tab=search&type=term') . '">backup plugin</a>.</p>';
    echo '<p>Use snapshots to find out what changes a plugin made to your database or to quickly restore the dev environment after testing database related changes. Restoring a snapshot does not affect other snapshots, or WP Reboot settings.</p>';
  ...
  {
    $base_url = 'https://binarypoets.net/wp-reboot';
    ...
    $parts = array_merge(array('utm_source' => 'wp-reboot-free', 'utm_medium' => 'plugin', 'utm_content' => $placement, 'utm_campaign' => 'wp-reboot-free-v' . $this->version), $params);
  ...
  {
    $snapshots = get_option('wp-reboot-snapshots', array());
  ...
  function do_create_snapshot($name = '')
  {
    ...
    do_action('wp_reboot_create_snapshot', $uid, $snapshot);
  ...
  function do_delete_snapshot($uid = '')
  {
    ...
    do_action('wp_reboot_delete_snapshot', $uid, $snapshot_copy);
  ...
  function do_export_snapshot($uid = '')
  {
    ...
    try {
      ...
      $world_dumper->dump(trailingslashit(WP_CONTENT_DIR) . $this->snapshots_folder . '/wp-reboot-snapshot-' . $uid . '.sql.gz', $uid . '_');
    } catch (Shuttle_Exception $e) {
      return new WP_Error(1, 'Couldn\'t create snapshot: ' . $e->getMessage());
    }

    do_action('wp_reboot_export_snapshot', 'wp-reboot-snapshot-' . $uid . '.sql.gz');

    return 'wp-reboot-snapshot-' . $uid . '.sql.gz';
  } // export_snapshot
  ...
  function do_restore_snapshot($uid = '')
  {
    ...
    do_action('wp_reboot_restore_snapshot', $uid);
  ...
  function verify_snapshot_integrity($uid)
  {
    ...
    if (is_array($table_status)) {
      ...
      if ($tbl_core != $snapshot['tbl_core'] || $tbl_custom != $snapshot['tbl_custom']) {
        return new WP_Error(1, 'Snapshot data has been compromised. Saved metadata does not match data in the DB. Contact WP Reboot support if data is critical, or restore it via a MySQL GUI.');
      }
    } else {
      return new WP_Error(1, 'Can\'t get table status data.');
    }
  ...
  {
    delete_option('wp-reboot');
    delete_option('wp-reboot-snapshots');
  } // uninstall
...
if (is_admin() || WP_Reboot::is_cli_running()) {
  global $wp_reboot;
  $wp_reboot = WP_Reboot::getInstance();
  add_action('plugins_loaded', array($wp_reboot, 'load_textdomain'));
  register_uninstall_hook(__FILE__, array('WP_Reboot', 'uninstall'));
}
